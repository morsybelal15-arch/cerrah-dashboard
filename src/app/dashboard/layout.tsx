...
import { Activity, FileText, User, UserPlus, Monitor, LogOut, ArrowRight } from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // روابط المريض
  const patientLinks = [
    { name: 'متابعة ومهام اليوم', icon: Activity, path: '/patient' },
    { name: 'الروشتة والدايت', icon: FileText, path: '/patient/prescription' },
    { name: 'مجسم التطور (3D)', icon: User, path: '/patient/3d-model' },
  ];

  // روابط الطبيب
  const doctorLinks = [
    { name: 'نظرة عامة', icon: Activity, path: '/doctor' },
    { name: 'تسجيل مريض جديد', icon: UserPlus, path: '/doctor/new-patient' },
    { name: 'الروشتة والدايت', icon: FileText, path: '/doctor/prescriptions' },
    { name: 'المتابعة الذكية', icon: Monitor, path: '/doctor/smart-followup' },
  ];
          ...
          {isDoctor ? (
            <div className="flex items-center gap-3 mb-6 px-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#8DC63F] font-bold">مدير النظام</p>
              </div>
            </div>
          ) : null}
